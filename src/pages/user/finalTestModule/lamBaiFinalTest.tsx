...
const LamBaiFinalTest = () => {
  ...
  // Timer state - use hackathon duration
  ...
  useEffect(() => {
    if (hackathon && !timerStarted) {
      setTimeLeft((hackathon.durationMinutes || 60) * 60);
  ...
  const fetchFinalTest = async () => {
    try {
      ...
      if (data.success) {
        ...
        // Set initial code from first problem
      ...
      notification.error({ message: 'Lỗi', description: 'Không thể tải Final Test' });
    } finally {
      setLoading(false);
    }
  };
...
int main() {
  ...
  const handleRunCode = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const currentProblem = hackathon?.problems[currentQuestionIndex];
      
      // Call hackathon run API
      const response = await fetch(API_ENDPOINTS.hackathons.runProblem(finalTestId!), {
        method: 'POST',
        headers: {
      ...
      if (data.success && data.data?.results) {
        ...
        notification.error({ message: 'Lỗi', description: data.message || 'Không thể chạy code' });
      }
    } catch (error) {
  ...
  const handleSubmitQuestion = async () => {
    ...
    try {
      ...
      const response = await fetch(API_ENDPOINTS.hackathons.submitProblem(finalTestId!), {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleAutoSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      // Submit all problems
      for (const problem of hackathon?.problems || []) {
        await fetch(API_ENDPOINTS.hackathons.submitProblem(finalTestId!), {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
      ...
      setTimeout(() => {
        navigate(`/user/final-test/${finalTestId}/ket-qua`);
      }, 1500);
    } catch (error) {
  ...
  const goToNextQuestion = () => {
    if (currentQuestionIndex < (hackathon?.problems.length || 0) - 1) {
  ...
  const currentProblem = hackathon?.problems[currentQuestionIndex];
  const publicTestCases = currentProblem?.testcases?.filter(tc => tc.isPublic) || [];
    ...
    return (
      <div className={cn('h-screen flex items-center justify-center', isDark ? 'bg-slate-950' : 'bg-slate-50')}>
        <div className="flex flex-col items-center gap-4">
          ...
          <p className={cn('text-lg', isDark ? 'text-slate-400' : 'text-slate-600')}>Đang tải Final Test...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={cn('h-screen flex items-center justify-center', isDark ? 'bg-slate-950' : 'bg-slate-50')}>
        <div className="text-center">
          ...
          <p className={cn('text-xl font-semibold mb-4', isDark ? 'text-white' : 'text-slate-900')}>Không tìm thấy Final Test</p>
          <Button onClick={() => navigate(-1)} className="bg-amber-500 hover:bg-amber-600">
      ...
      <header className={cn(
        ...
        <div className="flex items-center justify-between px-6 py-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className={cn(
              ...
              <span>Câu {currentQuestionIndex + 1}/{hackathon.problems.length}</span>
            </div>
        ...
        <div className={cn('px-6 pb-2 flex gap-2 overflow-x-auto', isDark ? 'bg-slate-950' : 'bg-white')}>
          {hackathon.problems.map((p, idx) => (
            ...
            >
              Câu {idx + 1}: {p.title.substring(0, 20)}...
            </button>
          ))}
        </div>
      </header>
          ...
          {showConsole && (
            <div className={cn(
              ...
              <div className={cn(
                ...
                <div className="flex items-center gap-2">
                  <Terminal className={cn('w-4 h-4', isDark ? 'text-amber-400' : 'text-amber-600')} />
              ...
              <div className="flex-1 overflow-y-auto p-4">
                {testResults.length === 0 && !runningCode ? (
                  ...
                  <div className="flex items-center justify-center py-8">
                    <Loader2 className={cn('w-6 h-6 animate-spin', isDark ? 'text-amber-400' : 'text-amber-600')} />
          ...
          <div className={cn(
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                disabled={currentQuestionIndex === hackathon.problems.length - 1}
                variant="ghost"
                size="sm"
                className={cn(
                  'gap-1 text-sm',
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className={cn(
            ...
            <p className={cn(
              ...
              Bạn đã hoàn thành <strong>{hackathon.problems.length}</strong> bài tập. 
...
export default LamBaiFinalTest;
