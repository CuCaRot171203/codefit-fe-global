...
const CertificatePage = () => {
  ...
  const handleCopyLink = () => {
    ...
    const verifyUrl = getVerifyUrl(selectedCertificate.id);
  ...
  const downloadCertificatePDF = async () => {
    ...
    try {
      ...
      doc.text('CODEFIT CERTIFICATION', pageWidth / 2, 35, { align: 'center' });
      ...
      doc.setFontSize(32);
      doc.setFont('helvetica', 'bold');
      doc.text('CHỨNG CHỈ HOÀN THÀNH', pageWidth / 2, 60, { align: 'center' });

      // Subtitle
      doc.setTextColor(148, 163, 184);
      doc.setFontSize(14);
      doc.setFont('helvetica', 'italic');
      doc.text('Chứng nhận học viên', pageWidth / 2, 70, { align: 'center' });

      // User name
      doc.setTextColor(245, 158, 11);
      doc.setFontSize(28);
      doc.setFont('helvetica', 'bold');
      doc.text(selectedCertificate.userName || 'Học viên', pageWidth / 2, 90, { align: 'center' });

      // Description
      doc.setTextColor(148, 163, 184);
      doc.setFontSize(11);
      doc.setFont('helvetica', 'normal');
      doc.text('Đã hoàn thành xuất sắc chương trình đào tạo chuyên sâu', pageWidth / 2, 105, { align: 'center' });
      doc.text('và đạt được đầy đủ các tiêu chuẩn kỹ thuật yêu cầu trong khóa học:', pageWidth / 2, 112, { align: 'center' });

      // Course title box
      doc.setFillColor(30, 64, 175);
      doc.roundedRect(pageWidth / 2 - 60, 118, 120, 18, 3, 3, 'F');
      doc.setTextColor(255, 255, 255);
      doc.setFontSize(14);
      doc.setFont('helvetica', 'bold');
      doc.text(selectedCertificate.courseTitle || 'Khóa học CodeFit', pageWidth / 2, 128, { align: 'center' });
      ...
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      doc.text('NGÀY CẤP', 50, bottomY);
      doc.setTextColor(255, 255, 255);
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
      doc.text(formatDate(selectedCertificate.issuedAt), 50, bottomY + 8);

      // QR Code
      if (qrCodeUrl) {
        doc.addImage(qrCodeUrl, 'PNG', pageWidth / 2 - 15, bottomY - 10, 30, 30);
      }
      ...
      doc.setFontSize(8);
      doc.setFont('helvetica', 'normal');
      doc.text(`ID: ${formatCertificateId(selectedCertificate.id)}`, pageWidth / 2, bottomY + 25, { align: 'center' });

      // Signature
      doc.setTextColor(148, 163, 184);
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      doc.text('CHỮ KÝ GIẢNG VIÊN', pageWidth - 50, bottomY);
      doc.setTextColor(191, 219, 254);
      doc.setFontSize(16);
      doc.setFont('helvetica', 'italic');
      doc.text('CodeFit Team', pageWidth - 50, bottomY + 10);

      // Footer
      doc.setTextColor(100, 116, 139);
      doc.setFontSize(8);
      doc.setFont('helvetica', 'normal');
      doc.text(`Xác thực tại: ${getVerifyUrl(selectedCertificate.id)}`, pageWidth / 2, pageHeight - 20, { align: 'center' });

      // Download
      doc.save(`CodeFit_ChungChi_${formatCertificateId(selectedCertificate.id)}.pdf`);
  ...
  if (certificates.length === 0) {
    return (
      <div className={cn('min-h-screen p-8', isDark ? 'bg-slate-900' : 'bg-amber-50/30')}>
        <div className="max-w-6xl mx-auto">
          <div className="mb-12">
            <h1
              className={cn(
                'text-4xl lg:text-5xl font-headline font-bold tracking-tight mb-2',
          ...
          >
            <Award
              className={cn('w-16 h-16 mx-auto mb-4 opacity-50', isDark ? 'text-slate-500' : 'text-slate-400')}
            />
            <h2 className={cn('text-xl font-bold mb-2', isDark ? 'text-white' : 'text-slate-900')}>
              Chưa có chứng chỉ nào
            </h2>
            <p className={cn('text-lg mb-6', isDark ? 'text-slate-400' : 'text-slate-500')}>
  ...
  const verifyUrl = cert ? getVerifyUrl(cert.id) : '';

  return (
    <div className={cn('min-h-screen p-8', isDark ? 'bg-slate-900' : 'bg-amber-50/30')}>
      {/* QR Modal - Custom Portal */}
      {qrModalOpen && cert && createPortal(
        <div 
          className={cn(
              ...
              <button
                ...
                className={cn(
                  ...
                  selectedCertificate?.id === c.id
                    ? isDark ? 'bg-amber-500 text-slate-900' : 'bg-amber-500 text-white'
